...
export default function ProgressCharts({ grid }) {
  ...
  return (
    <div className="
      ...
      shadow-xl
    ">
      <h2 className="text-lg font-semibold text-sky-900 dark:text-teal-200 mb-4">
      ...
      <svg viewBox={`0 0 ${w} ${h}`} width="100%">
        <defs>
          <linearGradient id="waveFill" x1="0" y1="0" x2="0" y2="1">
            <stop
              offset="0%"
              stopColor={theme === "dark" ? "#2dd4bf" : "#38bdf8"}
              stopOpacity={theme === "dark" ? 0.45 : 0.6}
            />
             <stop
              offset="100%"
              stopColor={theme === "dark" ? "#2dd4bf" : "#38bdf8"}
              stopOpacity={theme === "dark" ? 0.08 : 0.12}
            />
          </linearGradient>
        </defs>
...
function Stat({ label, value }) {
  return (
    ...
    ">
      <p className="text-sm text-sky-600 dark:text-teal-300">
        {label}
      </p>
      <h3 className="text-2xl font-bold text-sky-900 dark:text-teal-100">
        {value}
      </h3>
    </div>
  );
